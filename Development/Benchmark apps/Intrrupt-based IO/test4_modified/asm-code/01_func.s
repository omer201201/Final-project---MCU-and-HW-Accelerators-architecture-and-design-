...
elif1_l1:
	li	 t2,1
	bne  t4,t2,elif2_l1
	srli t0,t0,2									# if(t4 == 1) duty_cycle=0.25
elif2_l1:
	li	 t2,2
	bne  t4,t2,elif3_l1
	srli t0,t0,3									# if(t4 == 2) duty_cycle=0.125
elif3_l1:
	li	 t2,3
	bne  t4,t2,endif_l1
	srli t0,t0,4									# if(t4 == 3) duty_cycle=0.0625
endif_l1:
	li 	 t6,BTCMPR1
	...
	ret
#-----------------------------------------------------------------
#	Function: Basic Timer input capture mode configuration
# arguments: none
# return value: none
#-----------------------------------------------------------------
bt_capture_config:
...
# return value: none
#-----------------------------------------------------------------
# [MOD] The original wrote BTCTL1=BTHOLD_BTCLR (0x24), which has BTINT=00.
# That undid the BTINT=2 set by bt_capture_config, so a capture event could no
# longer drive BTIFG -- and it left BTHOLD=1, freezing BTCNT so nothing could
# be timed.  Corrected sequence: clear the counter with BTINT=2 still selected,
# arm the capture input low, then RELEASE the counter so it times the work.
capture_init:
	# BTCNT clear and hold, capture event still selected as the BTIFG source
  ...
  li   t6,BTCTL1
	...
	# [MOD] release BTHOLD/BTCLR so BTCNT counts SMCLK cycles during the
	# measured routine.  BTSSEL=0 -> one tick per SMCLK, so BTCAPR ends up
	# holding the elapsed CPU-cycle count directly.
	li 	 t0,BTINT2								# BTCTL1=(BTHOLD=0,BTCLR=0,BTSSEL=0,BTINT=2), start counting
  li   t6,BTCTL1
	sw   t0,0(t6)
...
# return value: none
#-----------------------------------------------------------------
# [MOD] The original wrote CAPMD1_CAPISEL3 here too (GND again), so the capture
# source never changed and no edge was ever produced.  Writing CAPISEL=2 (VCC)
# drives the selected source '0'->'1' = the rising edge the unit is armed for,
# which latches BTCNT into BTCAPR and raises BTIFG.
capture:
	li 	 t0,CAPMD1_CAPISEL2				# set the input signal to VCC -> rising edge -> BTCAPR=BTCNT
	li 	 t6,BTCTL2
	sw   t0,0(t6)
...
# return value: none
#-----------------------------------------------------------------
# [MOD] The original loop was two bugs in one: the guard was `blt t4,a3`
# (exit while the counter is still BELOW the size -- inverted), and there was
# no backward branch at all, so the body could never repeat.  Entering with
# t4=0 and a3=SIZE it branched straight to the ret and divarr stayed all-zero.
div_arrays:
...
for_l:
	bge	t4,a3,endfor_l						# [MOD] exit when iterator reaches SIZE
	lw	t1,0(s8)
	lw	t2,0(s9)
	div t3,t1,t2
	sw  t3,0(s10)
...
# return value: none
#-----------------------------------------------------------------
# [MOD] same two fixes as div_arrays
rem_arrays:
	mv  t4, zero									# initialization of the array iterator t4
	mv 	s8,a1											# s8 is the arr1_ptr
	mv 	s9,a2											# s9 is the arr2_ptr
...
for_l1:
	bge	t4,a3,endfor_l1						# [MOD] exit when iterator reaches SIZE
	lw	t1,0(s8)
	lw	t2,0(s9)
	rem t3,t1,t2
	sw  t3,0(s10)
	...
	addi s8,s8,4								  # arr1_ptr++
	addi s9,s9,4								  # arr2_ptr++
	addi s10,s10,4								# remarr_ptr++
	addi t4,t4,1									# iterator incrementation
	j    for_l1										# [MOD] loop back
endfor_l1:
	ret
