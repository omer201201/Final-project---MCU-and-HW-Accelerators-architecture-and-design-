...
main:
	...
	# [MOD] Clear the HEXs ONCE here, at startup.  In the original this pair of
	# instructions sat at the top of FSM_START, so every pass of the FSM loop
	# (~2 us) re-zeroed a0 and blanked the display.  a0 could never accumulate
	# and the STATE1 count was invisible -- the HEXs just read 000000.
	li  a0,0
	call print2HEXsArr						# clear the HEXs array {HEX5..HEX0}
...
if_l4:
	...
	call capture
	mv   fp,zero									# goto idle state
	j    END											# [MOD] the original fell through into
																# elif_l4 and ran the rem branch as well,
																# so a7&1 selected nothing.
elif_l4:
...
BT_ISR:
	...
	call print2HEXsArr						# print the value of a0 onto the HEXs array
elif1_l2:
	li	 t2,3
	la	 t3,state
	lw   t3,0(t3)
