# ------------------------------------------------------------------------------
# run_intr_test1.do  --  Intrrupt-based IO/test1 - KEY1/2/3 interrupts
#
# Run from the SUBMISSION ROOT:   vsim -c -do SIM/RV32IMscMCU/run_intr_test1.do
# ------------------------------------------------------------------------------
set BM intr_test1
...
# SW0 MUST be 1 before reset is released. test1 reads PORT_SW at startup and
# only the SW0=1 branch reaches 'ori gp,gp,0x01' (EINT) -- on the SW0=0 path
# the 'j STATE1' jumps straight past it, GIE is never set, and INTR is raised
# but never serviced. SW0 also selects the long delay (see the benchmark ReadMe).
force -deposit /tb_RV32IMscMCU/SW 00000001
