...
--==================================================================================================================
	...
	----------------------------------------------------------------------------
	-- DIVCLK : the accelerator's fast clock domain (project definition Fig.1
	-- clock tree, Fig.3, and p.9 "32 DIVCLK cycles (fast clock)").
	--   MCLK   = 50MHz * G_PLL_MUL      / G_PLL_DIV      =  25 MHz
	--   DIVCLK = 50MHz * G_PLL_MUL_DIV  / G_PLL_DIV_DIV  = 150 MHz  (= 6 x MCLK)
	-- An integer ratio is chosen deliberately: it keeps the two domains
	-- phase-related, and 150MHz leaves margin under the divider's measured
	-- standalone ceiling of ~195MHz on this device.
	----------------------------------------------------------------------------
	constant G_PLL_DIV_DIV	 			: NATURAL	:= 1;											-- DIVCLK divide  (relevant only when G_MODELSIM=0)
	constant G_PLL_MUL_DIV	 			: NATURAL	:= 3;											-- DIVCLK multiply(relevant only when G_MODELSIM=0)
